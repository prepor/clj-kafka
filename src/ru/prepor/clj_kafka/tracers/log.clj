...
(defcomponent log-tracer []
  []
  trace/Tracer
  (started
   [this group topic partition init-offset]
   (log/infof "%s(%s:%s) partition started with init offset: %s"
              group topic partition init-offset))
  (state-changed
   [this group topic partition state]
  ...
  (wait-acks-progress
   [this group topic partition consumed-offset acked-offset]
